...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`site_settings\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`title\` text DEFAULT 'vcontheroad' NOT NULL,
  	\`suffix\` text DEFAULT '.cv',
  	\`favicon_id\` integer,
  	\`short_name\` text DEFAULT 'VCCV' NOT NULL,
  	\`updated_at\` text,
  	\`created_at\` text,
  ...
  await db.run(sql`CREATE TABLE \`site_settings_locales\` (
  	\`subtitle\` text DEFAULT 'Vincent Chow On The Road',
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`_locale\` text NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`about_meta_meta_image_idx\` ON \`about_locales\` (\`meta_image_id\`,\`_locale\`);`)
